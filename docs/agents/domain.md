...
- `docs/adr/` at the repo root — one file per architectural decision, numbered sequentially.
  Superseded ADRs are marked in their own Status section, not deleted.
...
- These skills read `docs/adr/` for past decisions in the area being touched, and should not
  re-litigate a settled decision without flagging that they're doing so and why. Check an ADR's
  Status line first — a Superseded ADR's reasoning is historical context, not current direction.
